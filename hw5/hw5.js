...
$(function () {
    $.ajax({
        ...
        success: function (Data) {
            ...
            for(var i=1;i<=6;i++)
            {
                $("#ticketype").append("<option>"+i+type[i-1]+"</option>");
            }
            for(var i=1;i<=8;i++)
            {
                $("#fareclass").append("<option>"+i+fare[i-1]+"</option>");
            }
    ...
    $.ajax({
        ...
        success: function (Data) {
            $("#price").click(function(){
                var start=$("#start").val();
                var end=$("#end").val();
                var ticketype=$("#ticketype").val().substring(0,1);
                var fareclass=$("#fareclass").val().substring(0,1);
                if(start==end){
